...
public abstract class Widget : IWidget, IJsonWritable, IVisibleWidget, IDisableCallback
{
	...
	{
		IJsonWriter val = binding.PatchBegin();
		if (changes == WidgetChanges.Path)
		{
			WritePatchPath(val, path, "path");
			JsonWriterExtensions.Write<PathSegment>(val, widget.path);
		}
		else if ((changes & WidgetChanges.TotalProperties) != WidgetChanges.None && (changes & ~WidgetChanges.TotalProperties) == 0)
		{
			WritePatchPath(val, path, "props");
			val.TypeBegin(widget.propertiesTypeName);
			widget.WriteProperties(val);
			val.TypeEnd();
		}
		else if (changes == WidgetChanges.Children)
		{
			WritePatchPath(val, path, "children");
			JsonWriterExtensions.Write<IWidget>(val, widget.visibleChildren);
		}
		else
		{
			WritePatchPath(val, path);
			JsonWriterExtensions.Write<IWidget>(val, widget);
		}
		binding.PatchEnd();
	}
	...
	{
		JsonWriterExtensions.ArrayBegin(writer, 2 * path.Count - 1);
	...
	{
		JsonWriterExtensions.ArrayBegin(writer, 2 * path.Count);
		for (int i = 0; i < path.Count - 1; i++)
		{
			writer.Write(path[i]);
			writer.Write("children");
	...
	{
		JsonWriterExtensions.ArrayBegin(writer, 2 * path.Count + 3);
		for (int i = 0; i < path.Count - 1; i++)
		{
			writer.Write(path[i]);
			writer.Write("children");
