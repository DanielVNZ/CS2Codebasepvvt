...
public class EditorGenerator : IEditorGenerator
{
	...
	private static readonly CustomSerializationPolicy kMemberFilter = new CustomSerializationPolicy("EditorGenerator", true, (Func<MemberInfo, bool>)((MemberInfo member) => member is FieldInfo { IsPublic: not false } fieldInfo && !MemberInfoExtensions.IsDefined<NonSerializedAttribute>((ICustomAttributeProvider)fieldInfo, true) && !MemberInfoExtensions.IsDefined<HideInInspector>((ICustomAttributeProvider)fieldInfo) && !MemberInfoExtensions.IsDefined<HideInEditorAttribute>((ICustomAttributeProvider)fieldInfo) && fieldInfo.Name != "active" && fieldInfo.Name != "dirty" && fieldInfo.Name != "m_Dirty" && (!typeof(PrefabBase).IsAssignableFrom(fieldInfo.DeclaringType) || fieldInfo.Name != "components")));
	...
	public IEnumerable<IWidget> BuildMembers(IValueAccessor accessor, int level, string parentPath)
	{
		...
		list.AddRange(FormatterUtilities.GetSerializableMembers(accessor.valueType, (ISerializationPolicy)(object)kMemberFilter));
	...
	{
		if (TypeExtensions.InheritsFrom(type, typeof(PrefabBase)))
		{
			return typeof(Object).GetMember("name", MemberTypes.Property, BindingFlags.Instance | BindingFlags.Public);
		}
	...
	private IWidget BuildMember(IValueAccessor parent, MemberInfo member, int level, string parentPath)
	{
		...
		if (widget is INamed named)
		{
			InspectorNameAttribute attribute = MemberInfoExtensions.GetAttribute<InspectorNameAttribute>((ICustomAttributeProvider)member);
			EditorNameAttribute attribute2 = MemberInfoExtensions.GetAttribute<EditorNameAttribute>((ICustomAttributeProvider)member);
		...
		if (widget is ITooltipTarget tooltipTarget)
		{
			TooltipAttribute val = default(TooltipAttribute);
			tooltipTarget.tooltip = (ReflectionUtils.TryGetAttribute<TooltipAttribute>(member, ref val, false) ? LocalizedString.IdWithFallback(GetMemberTooltipLocaleId(member), val.tooltip) : LocalizedString.Id(GetMemberTooltipLocaleId(member)));
		}
		return widget;
	}
