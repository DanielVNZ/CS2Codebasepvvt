...
public class QuantityObjectMissingSystem : GameSystemBase
{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			for (int i = 0; i < nativeArray2.Length; i++)
			{
				...
				if (m_PrefabQuantityObjectData.HasComponent(prefabRef.m_Prefab))
				{
					...
					((ParallelWriter)(ref m_CommandBuffer)).AddComponent<Quantity>(unfilteredChunkIndex, val, default(Quantity));
	...
	protected override void OnCreate()
	{
		...
		m_Query = ((ComponentSystemBase)this).GetEntityQuery((ComponentType[])(object)new ComponentType[5]
		{
	...
	protected override void OnUpdate()
	{
		...
		if (!(((Context)(ref context)).version >= Version.resourcePileFixes) && !((EntityQuery)(ref m_Query)).IsEmptyIgnoreFilter)
		{
			QuantityObjectMissingJob quantityObjectMissingJob = new QuantityObjectMissingJob
			{
			...
			quantityObjectMissingJob.m_CommandBuffer = ((EntityCommandBuffer)(ref val)).AsParallelWriter();
			JobHandle val2 = JobChunkExtensions.ScheduleParallel<QuantityObjectMissingJob>(quantityObjectMissingJob, m_Query, ((SystemBase)this).Dependency);
