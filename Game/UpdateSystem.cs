...
public class UpdateSystem : GameSystemBase
{
	...
	protected virtual void OnBeginFrame(ScriptableRenderContext renderContext, Camera[] cameras)
	{
		foreach (IGPUSystem gPUSystem in m_GPUSystems)
		{
			if (gPUSystem.Enabled)
			{
				...
				gPUSystem.OnSimulateGPU(val);
				if (gPUSystem.IsAsync)
				{
					((ScriptableRenderContext)(ref renderContext)).ExecuteCommandBufferAsync(val, (ComputeQueueType)0);
				}
				else
				{
					((ScriptableRenderContext)(ref renderContext)).ExecuteCommandBuffer(val);
				}
				((ScriptableRenderContext)(ref renderContext)).Submit();
				val.Clear();
	...
	{
		RegisterGPUSystem(((ComponentSystemBase)this).World.GetOrCreateSystemManaged<SystemType>());
	}
	...
	{
		Register(++m_AddIndex, (ComponentSystemBase)(object)((ComponentSystemBase)this).World.GetOrCreateSystemManaged<SystemType>(), phase);
	}

	public void UpdateBefore<SystemType>(SystemUpdatePhase phase) where SystemType : ComponentSystemBase
	{
		Register(++m_AddIndex - 1000000, (ComponentSystemBase)(object)((ComponentSystemBase)this).World.GetOrCreateSystemManaged<SystemType>(), phase);
	}
	...
	{
		Register(++m_AddIndex - 1000000, (ComponentSystemBase)(object)((ComponentSystemBase)this).World.GetOrCreateSystemManaged<SystemType>(), (ComponentSystemBase)(object)((ComponentSystemBase)this).World.GetOrCreateSystemManaged<OtherType>(), phase);
	}
	...
	public void Update(SystemUpdatePhase phase)
	{
		...
		{
			currentPhase = phase;
			int2 val = m_UpdateRanges[(int)phase];
			for (int i = val.x; i < val.y; i++)
			{
				...
				{
					COSystemBase.baseLog.CriticalFormat(ex, "System update error during {0}->{1}:", (object)phase.ToString(), (object)((object)systemData.m_System).GetType().Name);
				}
			}
		}
		finally
		...
		try
		{
			...
			for (int i = val.x; i < val.y; i++)
			{
				...
				try
				{
					...
					{
						((GameSystemBase)(object)systemData.m_System).ResetDependency();
					}
					systemData.m_System.Update();
				}
				catch (Exception ex)
	...
	private void Refresh()
	{
		...
		while (num < m_Systems.Count)
		{
			...
			if (m_Updates.Count > count)
			{
				...
				{
					m_UpdateRanges.Add(int2.op_Implicit(count));
				}
