...
public class BpmAnalyzer
{
	...
	public static int AnalyzeBpm(AudioClip clip)
	{
		...
		log.InfoFormat("AnalyzeBpm audioClipName: {0}", (object)((Object)clip).name);
		int frequency = clip.frequency;
		log.InfoFormat("Frequency: {0}", (object)frequency);
		int channels = clip.channels;
		log.InfoFormat("Channels: {0}", (object)channels);
		...
		log.InfoFormat("Matched BPM: {0}", (object)num);
	...
	private static int SearchBpm(float[] volumeArr, int frequency, int splitFrameSize)
	{
		...
		for (int j = 60; j <= 400; j++)
		{
			...
			if (list.Count > 0)
			{
				...
				{
					num3 += list[k] * Mathf.Cos((float)k * 2f * (float)Math.PI * num5 / num2);
					num4 += list[k] * Mathf.Sin((float)k * 2f * (float)Math.PI * num5 / num2);
				}
