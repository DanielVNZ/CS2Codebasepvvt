...
public class PoliceCarAISystem : GameSystemBase
{
	...
	private struct PoliceCarTickJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			NativeArray<Game.Vehicles.PoliceCar> nativeArray7 = ((ArchetypeChunk)(ref chunk)).GetNativeArray<Game.Vehicles.PoliceCar>(ref m_PoliceCarType);
			...
			for (int i = 0; i < nativeArray.Length; i++)
			{
				...
				Tick(unfilteredChunkIndex, val, owner, transform, prefabRef, pathInformation, passengers, navigationLanes, serviceDispatches, isStopped, ref random, ref policeCar, ref car, ref currentLane, ref pathOwner, ref target);
		...
		private void ParkCar(int jobIndex, Entity entity, Owner owner, ref Game.Vehicles.PoliceCar policeCar, ref Car car, ref CarCurrentLane currentLane)
		{
			...
			if (m_PoliceStationData.TryGetComponent(owner.m_Owner, ref policeStation) && (policeStation.m_Flags & PoliceStationFlags.HasAvailablePatrolCars) == 0)
			{
		...
		private int BumpDispachIndex(Entity request)
		{
			...
			{
				result = policePatrolRequest.m_DispatchIndex + 1;
				m_ActionQueue.Enqueue(new PoliceAction
				{
		...
		private void RequestTargetIfNeeded(int jobIndex, Entity entity, ref Game.Vehicles.PoliceCar policeCar)
		{
			...
			else if ((policeCar.m_PurposeMask & (PolicePurpose.Emergency | PolicePurpose.Intelligence)) != 0)
			{
				...
				if ((m_SimulationFrameIndex & (num2 - 1)) == 5)
				{
					...
					((ParallelWriter)(ref m_CommandBuffer)).SetComponent<PoliceEmergencyRequest>(jobIndex, val2, new PoliceEmergencyRequest(entity, Entity.Null, 1f, policeCar.m_PurposeMask & (PolicePurpose.Emergency | PolicePurpose.Intelligence)));
		...
		private void ResetPath(int jobIndex, Entity vehicleEntity, ref Random random, PathInformation pathInformation, DynamicBuffer<ServiceDispatch> serviceDispatches, ref Game.Vehicles.PoliceCar policeCar, ref Car carData, ref CarCurrentLane currentLane, ref PathOwner pathOwner)
		{
			//IL_0006: Unknown result type (might be due to invalid IL or missing references)
			//IL_0007: Unknown result type (might be due to invalid IL or missing references)
			//IL_000c: Unknown result type (might be due to invalid IL or missing references)
		...
		private void TryReduceCrime(Entity building, float reduction)
		{
			...
			if (m_CrimeProducerData.TryGetComponent(building, ref crimeProducer) && crimeProducer.m_Crime > 0f)
			{
				m_ActionQueue.Enqueue(new PoliceAction
				{
		...
		private void TryReduceCrime(Entity vehicleEntity, PoliceCarData prefabPoliceCarData, ref CarCurrentLane currentLane, DynamicBuffer<CarNavigationLane> navigationLanes)
		{
			...
			if (!m_EdgeLaneData.TryGetComponent(currentLane.m_Lane, ref edgeLane) || !m_OwnerData.TryGetComponent(currentLane.m_Lane, ref owner))
			{
				return;
			}
			...
			for (int i = 0; i < navigationLanes.Length; i++)
			{
				...
				if ((carNavigationLane.m_Flags & Game.Vehicles.CarLaneFlags.Checked) != 0 || !m_OwnerData.TryGetComponent(carNavigationLane.m_Lane, ref owner2) || owner2.m_Owner != owner.m_Owner)
				{
					break;
				}
				carNavigationLane.m_Flags |= Game.Vehicles.CarLaneFlags.Checked;
			...
			crimeReductionRate *= math.abs(edgeLane.m_EdgeDelta.y - edgeLane.m_EdgeDelta.x);
			DynamicBuffer<ConnectedBuilding> val = default(DynamicBuffer<ConnectedBuilding>);
			if (m_ConnectedBuildings.TryGetBuffer(owner.m_Owner, ref val))
			{
				for (int j = 0; j < val.Length; j++)
				{
					TryReduceCrime(val[j].m_Building, crimeReductionRate);
				}
			}
		}
		...
		public void Execute()
		{
			...
			while (m_ActionQueue.TryDequeue(ref policeAction))
			{
				...
				case PoliceActionType.AddPatrolRequest:
				{
					...
					crimeProducer2.m_DispatchIndex = (byte)policeAction.m_DispatchIndex;
				...
				{
					PolicePatrolRequest policePatrolRequest = m_PolicePatrolRequestData[policeAction.m_Request];
					policePatrolRequest.m_DispatchIndex++;
					m_PolicePatrolRequestData[policeAction.m_Request] = policePatrolRequest;
					break;
				}
				}
			}
	...
	private struct TypeHandle
	{
		...
		public void __AssignHandles(ref SystemState state)
		{
			...
			__Game_Simulation_PolicePatrolRequest_RW_ComponentLookup = ((SystemState)(ref state)).GetComponentLookup<PolicePatrolRequest>(false);
	...
	protected override void OnUpdate()
	{
		...
		PoliceCarTickJob policeCarTickJob = new PoliceCarTickJob
		{
		...
		policeCarTickJob.m_CommandBuffer = ((EntityCommandBuffer)(ref val)).AsParallelWriter();
		policeCarTickJob.m_PathfindQueue = m_PathfindSetupSystem.GetQueue(this, 64).AsParallelWriter();
		policeCarTickJob.m_ActionQueue = actionQueue.AsParallelWriter();
		PoliceCarTickJob policeCarTickJob2 = policeCarTickJob;
		PoliceActionJob obj = new PoliceActionJob
		{
			m_SimulationFrame = m_SimulationSystem.frameIndex,
			m_PolicePatrolRequestData = InternalCompilerInterface.GetComponentLookup<PolicePatrolRequest>(ref __TypeHandle.__Game_Simulation_PolicePatrolRequest_RW_ComponentLookup, ref ((SystemBase)this).CheckedStateRef),
		...
		JobHandle val2 = JobChunkExtensions.ScheduleParallel<PoliceCarTickJob>(policeCarTickJob2, m_VehicleQuery, ((SystemBase)this).Dependency);
