		...
		public void Execute()
		{
			...
			while (val3.Length != 0)
			{
				...
				for (int m = 0; m < val5.Length; m++)
				{
					...
					float num = 0.5f;
					((bool3)(ref val7))._002Ector(false, true, false);
					if (edge.m_Start == resourceNodeItem.m_Node)
					{
						num = 0f;
						((bool3)(ref val7))._002Ector(true, false, false);
					}
					else if (edge.m_End == resourceNodeItem.m_Node)
					{
						num = 1f;
						((bool3)(ref val7))._002Ector(false, false, true);
					}
					else
					{
