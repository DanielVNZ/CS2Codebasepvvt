...
public class CarTrailerMoveSystem : GameSystemBase
{
	...
	private struct CarTrailerMoveJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			for (int i = 0; i < ((ArchetypeChunk)(ref chunk)).Count; i++)
			{
				...
				Moving moving = m_MovingData[val];
				DynamicBuffer<TransformFrame> val3 = m_TransformFrames[val];
				...
				for (int j = 1; j < val2.Length; j++)
				{
					...
					if (flag)
					{
						val4 = transform.m_Rotation;
						position = transform.m_Position;
						position += math.rotate(transform.m_Rotation, carTractorData.m_AttachPosition);
						position -= math.rotate(transform.m_Rotation, carTrailerData.m_AttachPosition);
					}
					else
					{
						...
						{
							float3 val5 = transform.m_Position + math.rotate(transform.m_Rotation, carTractorData.m_AttachPosition);
							float3 val6 = transform2.m_Position + math.rotate(transform2.m_Rotation, new float3(((float3)(ref carTrailerData.m_AttachPosition)).xy, carData.m_PivotOffset));
							val4 = transform.m_Rotation;
							float3 val7 = val5 - val6;
							val7 += moving.m_Velocity * (num * 0.25f);
							val7 -= moving2.m_Velocity * (num * 0.5f);
							if (MathUtils.TryNormalize(ref val7))
							{
								val4 = quaternion.LookRotationSafe(val7, math.up());
							}
							position = val5 - math.rotate(val4, carTrailerData.m_AttachPosition);
							break;
						}
						case TrailerMovementType.Locked:
							...
							position = transform.m_Position;
							position += math.rotate(transform.m_Rotation, carTractorData.m_AttachPosition);
							position -= math.rotate(transform2.m_Rotation, carTrailerData.m_AttachPosition);
							break;
						default:
							position = transform2.m_Position;
							val4 = transform2.m_Rotation;
							break;
						}
					}
					moving2.m_Velocity = (position - transform2.m_Position) / num;
					TransformFrame transformFrame = new TransformFrame
					{
						m_Position = (transform2.m_Position + position) * 0.5f,
						m_Velocity = moving2.m_Velocity,
						m_Rotation = math.slerp(transform2.m_Rotation, val4, 0.5f),
						m_Flags = val3[(int)m_TransformFrameIndex].m_Flags
					};
					transform2.m_Position = position;
					transform2.m_Rotation = val4;
					DynamicBuffer<TransformFrame> val8 = m_TransformFrames[vehicle];
					val8[(int)m_TransformFrameIndex] = transformFrame;
	...
	protected override void OnCreate()
	{
		...
		m_VehicleQuery = ((ComponentSystemBase)this).GetEntityQuery((ComponentType[])(object)new ComponentType[6]
		{
	...
	protected override void OnUpdate()
	{
		...
		JobHandle dependency = JobChunkExtensions.ScheduleParallel<CarTrailerMoveJob>(new CarTrailerMoveJob
		{
			...
			m_PrefabTractorData = InternalCompilerInterface.GetComponentLookup<CarTractorData>(ref __TypeHandle.__Game_Prefabs_CarTractorData_RO_ComponentLookup, ref ((SystemBase)this).CheckedStateRef),
			m_PrefabTrailerData = InternalCompilerInterface.GetComponentLookup<CarTrailerData>(ref __TypeHandle.__Game_Prefabs_CarTrailerData_RO_ComponentLookup, ref ((SystemBase)this).CheckedStateRef),
		...
		}, m_VehicleQuery, ((SystemBase)this).Dependency);
