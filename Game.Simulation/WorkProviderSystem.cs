...
public class WorkProviderSystem : GameSystemBase
{
	...
	private struct WorkProviderTickJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			{
				if (m_WorkplaceDatas.TryGetComponent((Entity)m_PrefabRefs[nativeArray[i]], ref workplaceData))
				{
					ref WorkProvider reference = ref CollectionUtils.ElementAt<WorkProvider>(nativeArray2, i);
					Entity val = Entity.Null;
					if (((ArchetypeChunk)(ref chunk)).Has<CompanyData>())
					{
					...
					else
					{
						...
						else if (((ArchetypeChunk)(ref chunk)).Has<Game.Buildings.School>())
						{
					...
					Workplaces workplaces2 = EconomyUtils.CalculateNumberOfWorkplaces(reference.m_MaxWorkers, workplaceData.m_Complexity, buildingLevel);
					...
					if (!((ArchetypeChunk)(ref chunk)).Has<Game.Objects.OutsideConnection>())
					{
						UpdateNotificationAndEfficiency(val, ref reference, bufferAccessor[i], workplaces2, freeWorkplaces, isDestroyed);
					}
				}
				else
				{
		...
		private void RefreshFreeWorkplace(int sortKey, Entity workplaceEntity, DynamicBuffer<Employee> employeeBuf, ref Workplaces freeWorkplaces)
		{
			...
			for (int i = 0; i < employeeBuf.Length; i++)
			{
				...
				{
					RemoveWorker(sortKey, employee.m_Worker);
					employeeBuf.RemoveAtSwapBack(i);
					i--;
		...
		private void UpdateNotificationAndEfficiency(Entity buildingEntity, ref WorkProvider workProvider, DynamicBuffer<Employee> employees, Workplaces maxWorkplaces, Workplaces freeWorkplaces, bool isDestroyed)
		{
			//IL_0051: Unknown result type (might be due to invalid IL or missing references)
			...
			if (m_Efficiencies.TryGetBuffer(buildingEntity, ref buffer))
			{
				...
				if (averageWorkforce > 0f)
				{
					...
					efficiency = val.x;
					efficiency2 = val.y;
					efficiency3 = ((averageWorkforce2 > 0f) ? (currentWorkforce / averageWorkforce2) : 1f);
				}
				else
				{
				...
				BuildingUtils.SetEfficiencyFactor(buffer, EfficiencyFactor.EmployeeHappiness, efficiency3);
			}
		}
