...
public class AdjustWaterConsumptionSystem : GameSystemBase
{
	...
	public struct AdjustWaterConsumptionJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			for (int i = 0; i < ((ArchetypeChunk)(ref chunk)).Count; i++)
			{
				...
				float num2 = (BuildingUtils.CheckOption(nativeArray2[i], BuildingOption.Inactive) ? (data.m_WaterConsumption * 0.1f) : data.m_WaterConsumption);
				num2 *= num;
				...
				if (wantedConsumption != num3)
				{
					...
					if (nativeArray3.Length != 0)
					{
						...
						{
							WaterPipeEdge waterPipeEdge = m_FlowEdges[consumerEdge];
							waterPipeEdge.m_FreshCapacity = num3;
							waterPipeEdge.m_SewageCapacity = num3;
							m_FlowEdges[consumerEdge] = waterPipeEdge;
						}
					}
				}
	...
	private struct UpdateEdgesJob : IJob
	{
		...
		public void Execute()
		{
			...
			{
				if (val.Add(val2) && m_NodeConnections.TryGetComponent(val2, ref waterPipeNodeConnection) && WaterPipeGraphUtils.TryGetFlowEdge(waterPipeNodeConnection.m_WaterPipeNode, m_SinkNode, ref m_FlowConnections, ref m_FlowEdges, out var entity, out var edge))
				{
					edge.m_SewageCapacity = (edge.m_FreshCapacity = GetNonConnectedBuildingConsumption(val2));
			...
			if (m_ConnectedBuildings.TryGetBuffer(roadEdge, ref val))
			{
				...
				try
				{
					...
					while (enumerator.MoveNext())
					{
						...
						if (m_Consumers.TryGetComponent(current.m_Building, ref waterConsumer) && !m_BuildingConnections.HasComponent(current.m_Building))
						{
							num += waterConsumer.m_WantedConsumption;
						}
					}
				}
				finally
	...
	{
		return ((AnimationCurve1)(ref feeParameters.m_WaterFeeConsumptionMultiplier)).Evaluate(relativeFee);
	}

	public static float GetFeeEfficiencyFactor(float relativeFee, in BuildingEfficiencyParameterData efficiencyParameters)
	{
		return ((AnimationCurve1)(ref efficiencyParameters.m_WaterFeeFactor)).Evaluate(relativeFee);
	}
	...
	protected override void OnUpdate()
	{
		...
		JobHandle val = JobChunkExtensions.ScheduleParallel<AdjustWaterConsumptionJob>(new AdjustWaterConsumptionJob
		{
			...
			m_FeeParameters = ((EntityQuery)(ref __query_1300465010_0)).GetSingleton<ServiceFeeParameterData>(),
			m_EfficiencyParameters = ((EntityQuery)(ref __query_1300465010_1)).GetSingleton<BuildingEfficiencyParameterData>(),
		...
		UpdateEdgesJob updateEdgesJob = new UpdateEdgesJob
		{
			...
			m_Consumers = InternalCompilerInterface.GetComponentLookup<WaterConsumer>(ref __TypeHandle.__Game_Buildings_WaterConsumer_RO_ComponentLookup, ref ((SystemBase)this).CheckedStateRef),
	...
	private void __AssignQueries(ref SystemState state)
	{
		...
		__query_1300465010_0 = ((EntityQueryBuilder)(ref val2)).Build(ref state);
		((EntityQueryBuilder)(ref val)).Reset();
		val2 = ((EntityQueryBuilder)(ref val)).WithAll<BuildingEfficiencyParameterData>();
		val2 = ((EntityQueryBuilder)(ref val2)).WithOptions((EntityQueryOptions)16);
		__query_1300465010_1 = ((EntityQueryBuilder)(ref val2)).Build(ref state);
		((EntityQueryBuilder)(ref val)).Reset();
