...
public struct FireEngineSelectData
{
	...
	public FireEngineSelectData(SystemBase system)
	{
		...
		m_FireEngineType = ((ComponentSystemBase)system).GetComponentTypeHandle<FireEngineData>(true);
	...
	public Entity CreateVehicle(ParallelWriter commandBuffer, int jobIndex, ref Random random, Transform transform, Entity source, Entity prefab, ref float2 extinguishingCapacity, RoadTypes roadType, bool parked)
	{
		//IL_0000: Unknown result type (might be due to invalid IL or missing references)
		//IL_0002: Unknown result type (might be due to invalid IL or missing references)
	...
	private Entity GetRandomVehicle(ref Random random, ref float2 extinguishingCapacity, RoadTypes roadType)
	{
		...
		for (int i = 0; i < m_PrefabChunks.Length; i++)
		{
			...
			NativeArray<FireEngineData> nativeArray2 = ((ArchetypeChunk)(ref chunk)).GetNativeArray<FireEngineData>(ref m_FireEngineType);
			...
			for (int j = 0; j < nativeArray2.Length; j++)
			{
				...
				float2 val = fireEngineData.m_ExtinguishingCapacity - extinguishingCapacity;
				float num3 = math.max(math.min(0f, val.x), val.y);
				if (num3 != num2)
				{
		...
		extinguishingCapacity = float2.op_Implicit(num);
		return result;
	}
