...
public struct HealthcareVehicleSelectData
{
	...
	public HealthcareVehicleSelectData(SystemBase system)
	{
		...
		m_AmbulanceType = ((ComponentSystemBase)system).GetComponentTypeHandle<AmbulanceData>(true);
		m_HearseType = ((ComponentSystemBase)system).GetComponentTypeHandle<HearseData>(true);
	...
	public Entity CreateVehicle(ParallelWriter commandBuffer, int jobIndex, ref Random random, Transform transform, Entity source, Entity prefab, HealthcareRequestType healthcareType, RoadTypes roadType, bool parked)
	{
		//IL_0000: Unknown result type (might be due to invalid IL or missing references)
		//IL_0002: Unknown result type (might be due to invalid IL or missing references)
	...
	private Entity GetRandomVehicle(ref Random random, HealthcareRequestType healthcareType, RoadTypes roadType)
	{
		...
		for (int i = 0; i < m_PrefabChunks.Length; i++)
		{
			...
			case HealthcareRequestType.Ambulance:
				if (!((ArchetypeChunk)(ref chunk)).Has<AmbulanceData>(ref m_AmbulanceType))
				{
					continue;
				}
				break;
			case HealthcareRequestType.Hearse:
				if (!((ArchetypeChunk)(ref chunk)).Has<HearseData>(ref m_HearseType))
				{
					continue;
				}
				break;
