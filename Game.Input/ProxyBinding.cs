...
public struct ProxyBinding : IEquatable<ProxyBinding>, IJsonWritable
{
	public class Comparer : IEqualityComparer<ProxyBinding>
	{
		...
		public int GetHashCode(ProxyBinding obj)
		{
			...
			{
				((HashCode)(ref val)).Add<string>(obj.m_MapName);
			}
			if ((m_Options & Options.ActionName) != 0)
			{
				((HashCode)(ref val)).Add<string>(obj.m_ActionName);
			}
			if ((m_Options & Options.Name) != 0)
			{
				((HashCode)(ref val)).Add<string>(obj.m_Name);
			}
			if ((m_Options & Options.Path) != 0)
			{
				((HashCode)(ref val)).Add<string>(obj.m_Path);
			}
			if ((m_Options & Options.Modifiers) != 0)
			{
				((HashCode)(ref val)).Add<IReadOnlyCollection<ProxyModifier>>((IReadOnlyCollection<ProxyModifier>)(object)obj.m_Modifiers, (IEqualityComparer<IReadOnlyCollection<ProxyModifier>>)m_ModifiersListComparer);
			}
			if ((m_Options & Options.Usages) != 0)
			{
				((HashCode)(ref val)).Add<Usages>(obj.usages, (IEqualityComparer<Usages>)Usages.Comparer.defaultComparer);
			}
			if ((m_Options & Options.Device) != 0)
			{
				((HashCode)(ref val)).Add<InputManager.DeviceType>(obj.m_Device);
			}
			if ((m_Options & Options.Component) != 0)
			{
				((HashCode)(ref val)).Add<ActionComponent>(obj.m_Component);
			}
	...
	public class Watcher : IDisposable
	{
		...
		{
			if (m_Action.TryGetBinding(m_Binding, out var foundBinding) && !comparer.Equals(m_Binding, foundBinding))
			{
	...
	public bool isModifiersRebindable
	{
		get
		{
			if (m_Source != null)
			{
				return m_Source.isModifiersRebindable;
			}
			return false;
		}
	}
	...
	public bool allowModifiers
	{
		get
		{
			if (m_Source != null)
			{
				return m_Source.allowModifiers;
			}
			return false;
		}
	}
	...
	internal OptionGroupOverride optionGroupOverride
	{
		get
		{
			...
			if (m_Alies != null && optionGroupOverride == OptionGroupOverride.None)
			{
	...
	public ConflictType hasConflicts
	{
		get
		{
			...
			{
				foreach (KeyValuePair<InputManager.DeviceType, ProxyComposite> composite in item.composites)
				{
	...
	internal bool isAlias => m_Alies != null;
	...
	public IEnumerable<string> ToHumanReadablePath()
	{
		if (!string.IsNullOrEmpty(m_Path))
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				yield return ControlPath.ToHumanReadablePath(proxyModifier.m_Path, (HumanReadableStringOptions)2);
			}
			yield return ControlPath.ToHumanReadablePath(m_Path, (HumanReadableStringOptions)2);
		}
	}
	...
	public void Write(IJsonWriter writer)
	{
		...
		JsonWriterExtensions.Write<ControlPath>(writer, ControlPath.Get(m_Path));
		writer.PropertyName("modifiers");
		JsonWriterExtensions.Write<ControlPath>(writer, (IList<ControlPath>)m_Modifiers.Select((ProxyModifier m) => ControlPath.Get(m.m_Path)).ToArray());
