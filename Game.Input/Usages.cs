...
public struct Usages : IEnumerable<int>, IEnumerable, IEquatable<Usages>
{
	public class Comparer : IEqualityComparer<Usages>
	{
		...
		public int GetHashCode(Usages usages)
		{
			...
			if (usages.m_Value != null)
			{
				...
				{
					((HashCode)(ref val)).Add<ulong>(usages.m_Value[i]);
				}
			}
	...
	public NameAndParameters parameters
	{
		get
		{
			...
			((NameAndParameters)(ref result)).name = "Usages";
			((NameAndParameters)(ref result)).parameters = new ReadOnlyArray<NamedValue>(this.Select((int u) => NamedValue.From<bool>(u.ToString(), true)).ToArray());
			return result;
		}
		set
		{
			...
			try
			{
				while (enumerator.MoveNext())
				{
					...
					if (int.TryParse(((NamedValue)(ref current)).name, out var result))
					{
						int index = result;
						PrimitiveValue value2 = ((NamedValue)(ref current)).value;
						this[index] = ((PrimitiveValue)(ref value2)).ToBoolean((IFormatProvider)null);
					}
				}
			}
			finally
	...
	public override string ToString()
	{
		if (m_Value != null)
		{
			return string.Join<int>('|', (IEnumerable<int>)this);
		}
		return "Empty";
	}
}
