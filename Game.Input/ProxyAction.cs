...
public class ProxyAction : IProxyAction
{
	...
	internal struct LinkInfo : IEquatable<LinkInfo>
	{
		...
		{
			return HashCode.Combine<ProxyAction, int>(m_Action, (int)m_Device);
		}
	}
	...
	public IEnumerable<ProxyBinding> bindings
	{
		get
		{
			...
			foreach (KeyValuePair<InputManager.DeviceType, ProxyComposite> composite in m_Composites)
			{
	...
	public unsafe T ReadValue<T>() where T : struct
	{
		...
		TriggerState* ptr = (TriggerState*)((byte*)state.actionStates + (nint)m_SourceAction.m_ActionIndexInState * (nint)System.Runtime.CompilerServices.Unsafe.SizeOf<TriggerState>());
		return state.ReadValue<T>(((TriggerState)ptr).bindingIndex, ((TriggerState)ptr).controlIndex, false);
	}
	...
	internal unsafe T ReadRawValue<T>(bool disableAll = true) where T : struct
	{
		...
		TriggerState* ptr = (TriggerState*)((byte*)state.actionStates + (nint)m_SourceAction.m_ActionIndexInState * (nint)System.Runtime.CompilerServices.Unsafe.SizeOf<TriggerState>());
		int bindingIndex = ((TriggerState)ptr).bindingIndex;
		ref BindingState reference = ref *(BindingState*)((byte*)state.bindingStates + (nint)bindingIndex * (nint)System.Runtime.CompilerServices.Unsafe.SizeOf<BindingState>());
		if (((BindingState)(ref reference)).isPartOfComposite)
		{
			reference = ref *(BindingState*)((byte*)state.bindingStates + (nint)((BindingState)(ref reference)).compositeOrCompositeBindingIndex * (nint)System.Runtime.CompilerServices.Unsafe.SizeOf<BindingState>());
		}
		int processorCount = ((BindingState)(ref reference)).processorCount;
		try
		{
			if (disableAll)
			{
				((BindingState)(ref reference)).processorCount = 0;
			}
			else
			{
				for (int i = 0; i < processorCount; i++)
				{
					if (state.processors[((BindingState)(ref reference)).processorStartIndex + i] is IDisableableProcessor disableableProcessor)
					{
		...
		finally
		{
			if (disableAll)
			{
				((BindingState)(ref reference)).processorCount = processorCount;
			}
			else
			{
				for (int j = 0; j < processorCount; j++)
				{
					if (state.processors[((BindingState)(ref reference)).processorStartIndex + j] is IDisableableProcessor disableableProcessor2)
					{
	...
	public unsafe float GetMagnitude()
	{
		...
		{
			TriggerState* ptr = (TriggerState*)((byte*)state.actionStates + (nint)m_SourceAction.m_ActionIndexInState * (nint)System.Runtime.CompilerServices.Unsafe.SizeOf<TriggerState>());
			if (((TriggerState)ptr).haveMagnitude)
			{
				return ((TriggerState)ptr).magnitude;
			}
		}
		return 0f;
	}
	...
	private void SourceOnStarted(CallbackContext context)
	{
		m_OnInteraction?.Invoke(this, (InputActionPhase)2);
	}

	private void SourceOnPerformed(CallbackContext context)
	{
		m_OnInteraction?.Invoke(this, (InputActionPhase)3);
	}

	private void SourceOnCanceled(CallbackContext context)
	{
		m_OnInteraction?.Invoke(this, (InputActionPhase)4);
	}
