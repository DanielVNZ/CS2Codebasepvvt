...
public class AssetImportPanel : EditorPanelSystemBase
{
	public class PrefabFactory : IPrefabFactory
	{
		...
		private T LoadOrCreateAsset<T>(string sourcePath, string rootMeshName) where T : PrefabBase
		{
			...
			PrefabAsset val = default(PrefabAsset);
			if (((IAssetDatabase)AssetDatabase.user).TryGetAsset<PrefabAsset>(SearchFilter<PrefabAsset>.ByCondition((Func<PrefabAsset, bool>)((PrefabAsset asset) => PathCompare(subPath, asset.subPath + "/" + ((AssetData)asset).name)), false), ref val) && val.Load() is T result)
			{
				return result;
			}
	...
	protected override void OnStartRunning()
	{
		...
		{
			base.log.Error(ex, (object)("Exception occured while trying to select project root or import directory " + m_SelectedProjectRoot + ", " + m_SelectedDirectory));
	...
	{
		Debug.Log((object)("Asset selected " + item.displayName));
	}
	...
	private IEnumerable<FileItem> GetAssets()
	{
		...
		IDictionary<AssetGroup<Asset>, Settings> dictionary = AssetImportPipeline.CollectDataToImport(m_SelectedProjectRoot, new string[1] { m_SelectedDirectory }, report);
		foreach (KeyValuePair<AssetGroup<Asset>, Settings> item in dictionary)
		{
			foreach (Asset item2 in item.Key)
			{
	...
	private static bool ReportProgress(string title, string info, float progress)
	{
		if (progress == 1f)
		{
			Debug.Log((object)"Import completed");
		}
		return false;
	}
	...
	private async void ImportAssets()
	{
		...
		{
			importing = true;
			await ImportAssets(m_SelectedProjectRoot, m_SelectedDirectory, m_SelectedTemplate, ((ComponentSystemBase)this).World, base.log);
		}
		finally
		{
			importing = false;
	...
	public static async Task<PrefabFactory> ImportAssets(string selectedProjectRoot, string selectedDirectory, PrefabBase selectedTemplate, World world, ILog log)
	{
		...
		try
		{
			...
			if (AssetImportPipeline.IsArtRootPath(selectedProjectRoot, new string[1] { selectedDirectory }, out var artProjectPath, out var artProjectRelativePaths))
			{
				...
				await AssetImportPipeline.ImportPath(artProjectPath, artProjectRelativePaths, (ImportMode)3, convertToVT: false, ReportProgress, prefabFactory);
				...
				{
					log.InfoFormat("Root prefab: {0} ({1})", (object)((Object)rootPrefab.prefab).name, (object)rootPrefab.source);
					string fileName = Path.GetFileName(rootPrefab.source);
					string text = "StreamingData~/" + fileName;
					if ((Object)(object)selectedTemplate != (Object)null)
					{
						...
						((AssetData)PrefabAssetExtensions.AddAsset(AssetImportPipeline.targetDatabase, AssetDataPath.Create(text, ((Object)prefabBase).name ?? "", (EscapeStrategy)2), (ScriptableObject)(object)prefabBase)).Save(false);
						orCreateSystemManaged.AddPrefab(prefabBase);
						orCreateSystemManaged2.ActivatePrefabTool(prefabBase);
					}
					else
					{
						if ((AssetData)(object)rootPrefab.prefab.asset == (IAssetData)null)
						{
							PrefabAssetExtensions.AddAsset(AssetImportPipeline.targetDatabase, AssetDataPath.Create(text, ((Object)rootPrefab.prefab).name ?? "", (EscapeStrategy)2), (ScriptableObject)(object)rootPrefab.prefab);
						}
						((AssetData)rootPrefab.prefab.asset).Save(true);
