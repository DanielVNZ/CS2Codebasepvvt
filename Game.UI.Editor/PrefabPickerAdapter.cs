...
public class PrefabPickerAdapter : ItemPicker<PrefabItem>.IAdapter, PopupSearchField.IAdapter, SearchField.IAdapter, ItemPickerFooter.IAdapter, FilterMenu.IAdapter
{
	...
	public void SetPrefabs([ItemCanBeNull] ICollection<PrefabBase> prefabs)
	{
		...
		foreach (PrefabBase prefab in prefabs)
		{
			...
			if ((Object)(object)prefab != (Object)null && displayPrefabTypeTooltip)
			{
				prefabItem.tooltip = LocalizedString.Value(((object)prefab).GetType().Name);
			}
			if ((Object)(object)prefab != (Object)null)
			{
				prefabItem.tags.AddRange(EditorPrefabUtils.GetPrefabTags(((object)prefab).GetType()));
				foreach (ComponentBase component in prefab.components)
				{
					prefabItem.tags.Add(((object)component).GetType().Name.ToLowerInvariant());
				}
	...
	{
		m_SelectedPrefab = m_Items.Select((PrefabItem item) => item.prefab).FirstOrDefault((PrefabBase prefab) => ((Object)prefab).name.Equals(name, comparisonType));
		return m_SelectedPrefab;
	}

	public void Update()
	{
		if ((Object)(object)m_SelectedPrefab != (Object)(object)m_SelectedItem?.prefab)
		{
			m_SelectedItem = m_Items.FirstOrDefault((PrefabItem item) => (Object)(object)item.prefab == (Object)(object)m_SelectedPrefab);
		}
	...
	private void UpdateFilteredItems()
	{
		...
		string[] array = m_SearchQuery.Split(' ', StringSplitOptions.None);
		...
		if (words.Length != 0 || tags.Length != 0 || incompleteTag != null)
		{
			...
			m_FilteredItems.AddRange(m_Items.Where(delegate(PrefabItem item)
			{
				...
				bool num = words.Length == 0 || words.All((string word) => ((Object)item.prefab).name.IndexOf(word, StringComparison.OrdinalIgnoreCase) != -1);
				string typeName = ((object)item.prefab).GetType().Name;
	...
	private static string GetIncompleteTag(string[] searchParts)
	{
		...
		string text = searchParts[searchParts.Length - 1];
