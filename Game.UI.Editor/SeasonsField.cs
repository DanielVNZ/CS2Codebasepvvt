...
public class SeasonsField : Widget
{
	public struct SeasonCurves : IJsonWritable, IJsonReadable
	{
		...
		public void Write(IJsonWriter writer)
		{
			if (m_Temperature != null)
			{
				...
				UnityWriters.Write(writer, m_Temperature);
				writer.PropertyName("precipitation");
				UnityWriters.Write(writer, m_Precipitation);
				writer.PropertyName("cloudiness");
				UnityWriters.Write(writer, m_Cloudiness);
				writer.PropertyName("aurora");
				UnityWriters.Write(writer, m_Aurora);
				writer.PropertyName("fog");
				UnityWriters.Write(writer, m_Fog);
				writer.TypeEnd();
			}
		}
		...
		public void Read(IJsonReader reader)
		{
			...
			UnityReaders.Read(reader, ref m_Temperature);
			reader.ReadProperty("precipitation");
			UnityReaders.Read(reader, ref m_Precipitation);
			reader.ReadProperty("cloudiness");
			UnityReaders.Read(reader, ref m_Cloudiness);
			reader.ReadProperty("aurora");
			UnityReaders.Read(reader, ref m_Aurora);
			reader.ReadProperty("fog");
			UnityReaders.Read(reader, ref m_Fog);
			reader.ReadMapEnd();
		}
	}
	...
	public class Bindings : IWidgetBindingFactory
	{
		...
		{
			yield return (IBinding)(object)new CallBinding<IWidget, ClimateSystem.SeasonInfo, int>(group, "onUpdateSeason", (Func<IWidget, ClimateSystem.SeasonInfo, int>)delegate(IWidget widget, ClimateSystem.SeasonInfo season)
			{
				int result = -1;
				...
				return result;
			}, pathResolver, (IReader<ClimateSystem.SeasonInfo>)(object)new ValueReader<ClimateSystem.SeasonInfo>());
		}
	}
	...
	protected override void WriteProperties(IJsonWriter writer)
	{
		...
		JsonWriterExtensions.Write<SeasonCurves>(writer, m_SeasonCurves);
	}
}
