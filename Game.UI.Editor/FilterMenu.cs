...
public class FilterMenu : Widget
{
	...
	public class Bindings : IWidgetBindingFactory
	{
		...
		{
			yield return (IBinding)(object)new TriggerBinding<IWidget, string, bool>(group, "toggleFilter", (Action<IWidget, string, bool>)delegate(IWidget widget, string filter, bool active)
			{
			...
			}, pathResolver, (IReader<string>)null, (IReader<bool>)null);
			yield return (IBinding)(object)new TriggerBinding<IWidget>(group, "clearFilters", (Action<IWidget>)delegate(IWidget widget)
			{
				if (widget is FilterMenu filterMenu)
				{
	...
	protected override void WriteProperties(IJsonWriter writer)
	{
		...
		JsonWriterExtensions.Write(writer, (IList<string>)adapter.availableFilters);
		writer.PropertyName("activeFilters");
		JsonWriterExtensions.Write(writer, (IList<string>)adapter.activeFilters);
	}
