...
[CompilerGenerated]
public class CoverageDebugSystem : BaseDebugSystem
{
	[BurstCompile]
	private struct CoverageGizmoJob : IJobChunk
	{
		[ReadOnly]
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			for (int i = 0; i < bufferAccessor.Length; i++)
			{
				...
				if (val.Length != 0)
				{
					...
					ServiceCoverage serviceCoverage = val[(int)m_Service];
					Color val2 = Color.Lerp(Color.red, Color.green, serviceCoverage.m_Coverage.x * 0.25f);
					Color val3 = Color.Lerp(Color.red, Color.green, serviceCoverage.m_Coverage.y * 0.25f);
					...
					((Segment)(ref val5))._002Ector(val4, val4 + new float3(0f, serviceCoverage.m_Coverage.x * 10f, 0f));
					...
					for (int j = 1; j <= num; j++)
					{
						...
						float num3 = math.lerp(serviceCoverage.m_Coverage.x, serviceCoverage.m_Coverage.y, val6.y);
					...
					for (int k = 1; k <= num2; k++)
					{
						...
						float num4 = math.lerp(serviceCoverage.m_Coverage.x, serviceCoverage.m_Coverage.y, val9.y);
	...
	private struct TypeHandle
	{
		...
		[ReadOnly]
		public BufferTypeHandle<ServiceCoverage> __Game_Net_ServiceCoverage_RO_BufferTypeHandle;
		...
		public void __AssignHandles(ref SystemState state)
		{
			...
			__Game_Net_ServiceCoverage_RO_BufferTypeHandle = ((SystemState)(ref state)).GetBufferTypeHandle<ServiceCoverage>(true);
		}
	}
	...
	protected override void OnCreate()
	{
		...
		m_CoverageGroup = ((ComponentSystemBase)this).GetEntityQuery((ComponentType[])(object)new ComponentType[4]
		{
			ComponentType.ReadOnly<ServiceCoverage>(),
		...
		((ComponentSystemBase)this).RequireForUpdate(m_CoverageGroup);
	...
	protected override JobHandle OnUpdate(JobHandle inputDeps)
	{
		...
		foreach (KeyValuePair<CoverageService, Option> item in m_CoverageOptions)
		{
			...
			{
				JobHandle val2 = JobChunkExtensions.ScheduleParallel<CoverageGizmoJob>(new CoverageGizmoJob
				{
					m_Service = item.Key,
					...
					m_CoverageType = InternalCompilerInterface.GetBufferTypeHandle<ServiceCoverage>(ref __TypeHandle.__Game_Net_ServiceCoverage_RO_BufferTypeHandle, ref ((SystemBase)this).CheckedStateRef),
					m_GizmoBatcher = m_GizmosSystem.GetGizmosBatcher(ref val3)
				}, m_CoverageGroup, JobHandle.CombineDependencies(inputDeps, val3));
