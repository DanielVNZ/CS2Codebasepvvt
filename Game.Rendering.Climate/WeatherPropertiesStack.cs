...
internal class WeatherPropertiesStack : IDisposable
{
	...
	private void CreateInterpolatedRepresentation()
	{
		foreach (Type item in from t in CoreUtils.GetAllTypesDerivedFrom<OverrideablePropertiesComponent>()
			...
			select t)
		{
			OverrideablePropertiesComponent overrideablePropertiesComponent = (OverrideablePropertiesComponent)(object)ScriptableObject.CreateInstance(item);
			OverrideablePropertiesComponent previous = (OverrideablePropertiesComponent)(object)ScriptableObject.CreateInstance(item);
			OverrideablePropertiesComponent target = (OverrideablePropertiesComponent)(object)ScriptableObject.CreateInstance(item);
			OverrideablePropertiesComponent overrideablePropertiesComponent2 = (OverrideablePropertiesComponent)(object)ScriptableObject.CreateInstance(item);
			OverrideablePropertiesComponent to = (OverrideablePropertiesComponent)(object)ScriptableObject.CreateInstance(item);
	...
	public void Dispose()
	{
		foreach (KeyValuePair<Type, InterpolatedProperties> component in components)
		{
			CoreUtils.Destroy((Object)(object)component.Value.current);
			CoreUtils.Destroy((Object)(object)component.Value.previous);
			CoreUtils.Destroy((Object)(object)component.Value.target);
			CoreUtils.Destroy((Object)(object)component.Value.from);
			CoreUtils.Destroy((Object)(object)component.Value.to);
		}
		components.Clear();
	}
}
