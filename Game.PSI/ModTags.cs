...
public static class ModTags
{
	...
	public static void GetTags(AssetData asset, HashSet<string> tags, HashSet<string> typeTags, HashSet<string> validTags)
	{
		...
		else
		{
			...
			if (val != null && val.Load() is PrefabBase prefab)
			{
		...
		while (tags.Count > kMaxTags)
		{
			...
			sLog.WarnFormat("Generated mod tags for {0} exceed max count, removing {1}", (object)asset.name, (object)text);
		}
	}

	private static void GetMapTags(MapMetadata map, HashSet<string> tags, HashSet<string> typeTags, HashSet<string> validTags)
	{
		if (validTags.Contains(((Metadata<MapInfo>)map).target.theme))
		{
			tags.Add(((Metadata<MapInfo>)map).target.theme);
		}
	}
	...
	{
		if (validTags.Contains(((Metadata<SaveInfo>)save).target.theme))
		{
			tags.Add(((Metadata<SaveInfo>)save).target.theme);
		}
	}
	...
	{
		Type type = ((object)assetData).GetType();
	...
	{
		if (((object)prefab).GetType() == typeof(StaticObjectPrefab))
		{
