...
public static class LauncherSettings
{
	private struct Settings
	{
		...
		public void Merge(SharedSettings gameSettings)
		{
			...
			ref System reference = ref system;
			ScreenResolution resolution = gameSettings.graphics.resolution;
			reference.refreshRate = ((RefreshRate)(ref resolution.refreshRate)).value;
	...
	public static void SaveSettings(SharedSettings gameSettings)
	{
		if (TryGetLauncherSettings(out var launcherSettings))
		{
			try
			{
				...
				string contents = JSON.Dump((object)launcherSettings, (EncodeOptions)0);
				File.WriteAllText(kLauncherSettingsPath, contents);
				log.Info((object)"Launcher settings saved successfully");
			}
			catch (Exception ex)
			{
				log.InfoFormat("Saving launcher settings failed: {0}", (object)ex);
			}
		}
	}
	...
	private static bool TryGetLauncherSettings(out Settings launcherSettings)
	{
		if (File.Exists(kLauncherSettingsPath))
		{
			try
			{
				Variant val = JSON.Load(File.ReadAllText(kLauncherSettingsPath));
				launcherSettings = val.Make<Settings>();
				log.Info((object)"Loaded launcher settings successfully");
				return true;
			}
			catch (Exception ex)
			{
				log.InfoFormat("Loading launcher settings failed: {0}", (object)ex);
			}
		}
		else
		{
			log.Info((object)"Launcher settings not present");
		}
