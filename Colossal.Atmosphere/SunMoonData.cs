...
public struct SunMoonData
{
	...
	private MoonCoords GetMoonCoords(double d)
	{
		...
		double num2 = math.radians(134.963 + 13.064993 * d);
		double num3 = math.radians(93.272 + 13.22935 * d);
		double l = num + math.radians(6.289 * math.sin(num2));
		double b = math.radians(5.128 * math.sin(num3));
		double distance = 385001.0 - 20905.0 * math.cos(num2);
	...
	public MoonIllumination GetMoonIllumination(JulianDateTime date)
	{
		...
		double num2 = math.acos(math.sin(sunCoords.declination) * math.sin(moonCoords.equatorialCoords.declination) + math.cos(sunCoords.declination) * math.cos(moonCoords.equatorialCoords.declination) * math.cos(sunCoords.rightAscension - moonCoords.equatorialCoords.rightAscension));
		double num3 = math.atan2(num * math.sin(num2), moonCoords.distance - num * math.cos(num2));
		double num4 = math.atan2(math.cos(sunCoords.declination) * math.sin(sunCoords.rightAscension - moonCoords.equatorialCoords.rightAscension), math.sin(sunCoords.declination) * math.cos(moonCoords.equatorialCoords.declination) - math.cos(sunCoords.declination) * math.sin(moonCoords.equatorialCoords.declination) * math.cos(sunCoords.rightAscension - moonCoords.equatorialCoords.rightAscension));
		return new MoonIllumination
		{
			fraction = (1.0 + math.cos(num3)) / 2.0,
			phase = 0.5 + 0.5 * num3 * (double)((!(num4 < 0.0)) ? 1 : (-1)) / Math.PI,
			angle = num4
		};
	}
}
