...
public class SunCalculator
{
	...
	{
		double num = th - a;
		return math.atan2(math.sin(num), math.cos(num) * math.sin(phi) - math.tan(d) * math.cos(phi)) + 3.1415927410125732;
	}

	private static double GetAltitude(double th, double a, double phi, double d)
	{
		double num = th - a;
		return math.asin(math.sin(phi) * math.sin(d) + math.cos(phi) * math.cos(d) * math.cos(num));
	}
