...
public class PetSystem : GameSystemBase
{
	...
	private struct PetJob : IJobChunk
	{
		...
		public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
		{
			...
			{
				Entity householdPet = nativeArray[i];
				CurrentTransport currentTransport = nativeArray2[i];
				if (m_PetData.HasComponent(currentTransport.m_CurrentTransport))
				{
					Pet pet = m_PetData[currentTransport.m_CurrentTransport];
					pet.m_HouseholdPet = householdPet;
					m_PetData[currentTransport.m_CurrentTransport] = pet;
				}
			}
		}
	...
	protected override void OnUpdate()
	{
		...
		PetJob petJob = new PetJob
		{
			...
			m_PetData = InternalCompilerInterface.GetComponentLookup<Pet>(ref __TypeHandle.__Game_Creatures_Pet_RW_ComponentLookup, ref ((SystemBase)this).CheckedStateRef)
		};
		((SystemBase)this).Dependency = JobChunkExtensions.Schedule<PetJob>(petJob, m_Query, ((SystemBase)this).Dependency);
	}
