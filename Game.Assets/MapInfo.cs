...
public class MapInfo : IJsonWritable, IContentPrerequisite, IComparable<MapInfo>
{
	...
	public void Write(IJsonWriter writer)
	{
		...
		writer.Write(UIExtensions.ToUri(thumbnail, MenuHelpers.defaultThumbnail, 0));
		writer.PropertyName("preview");
		writer.Write(UIExtensions.ToUri(preview, MenuHelpers.defaultPreview, 0));
		writer.PropertyName("theme");
		writer.Write(theme);
		writer.PropertyName("temperatureRange");
		MathematicsWriters.Write(writer, temperatureRange);
		...
		JsonWriterExtensions.Write<MapMetadataSystem.Resources>(writer, resources);
		writer.PropertyName("connections");
		JsonWriterExtensions.Write<MapMetadataSystem.Connections>(writer, connections);
		writer.PropertyName("contentPrerequisites");
		JsonWriterExtensions.Write(writer, contentPrerequisites);
