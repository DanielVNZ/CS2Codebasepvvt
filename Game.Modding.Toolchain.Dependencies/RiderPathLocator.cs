...
internal static class RiderPathLocator
{
	...
	public static RiderInfo[] GetAllRiderPaths()
	{
		...
		try
		{
			OperatingSystemFamily operatingSystemFamily = PlatformInfo.operatingSystemFamily;
		...
		if (!string.IsNullOrEmpty(environmentVariable))
		{
			...
			if (fileInfo.Exists)
			{
				...
				foreach (string text in array)
				{
					...
					{
						string path = text.Split('"', StringSplitOptions.None).Where((string item, int index) => index == 1).SingleOrDefault();
	...
	private static string GetRelativePathToBuildTxt()
	{
		...
		OperatingSystemFamily operatingSystemFamily = PlatformInfo.operatingSystemFamily;
		if ((int)operatingSystemFamily != 1)
		{
			if (operatingSystemFamily - 2 <= 1)
			{
				return "../../build.txt";
			}
