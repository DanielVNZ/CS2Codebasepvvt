	...
	private void ReadSettings()
	{
		...
		try
		{
			while (enumerator.MoveNext())
			{
				...
				if (m_PrefabSystem.TryGetPrefab<PrefabBase>(current, out var prefab) && ShownTutorials.TryGetValue(((Object)prefab).name, out var value))
				{
					if (value)
					{
		...
		{
			Entity val3 = val2[i];
			if (!m_PrefabSystem.TryGetPrefab<PrefabBase>(val3, out var prefab2) || !ShownTutorials.TryGetValue(((Object)prefab2).name, out var value2))
			{
				continue;
			}
			if (value2)
