...
public interface IToolchainDependency
{
	...
	public struct State
	{
		...
		{
			return HashCode.Combine<DependencyState, int?, LocalizedString>(m_State, m_Progress, m_Details);
		}
	}
	...
	static async Task<long> GetDownloadSizeAsync(string url, CancellationToken token, int timeout = 3000)
	{
		...
		catch (OperationCanceledException)
		{
			...
			{
				log.Error((object)("Get download size request timeout: " + url));
			}
			return -1L;
		}
		catch (Exception ex2)
		{
			log.Error(ex2, (object)("Get download size error: " + url));
			return -1L;
		}
	}
	...
	static void UpdateProcessEnvVarPathValue()
	{
		...
		string value = string.Join<string>(';', (IEnumerable<string>)allVars);
