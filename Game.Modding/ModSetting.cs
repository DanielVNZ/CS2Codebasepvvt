...
public abstract class ModSetting : Setting
{
	...
	{
		SettingsUIKeyboardBindingAttribute settingsUIKeyboardBindingAttribute = default(SettingsUIKeyboardBindingAttribute);
		string actionName;
		...
		SettingsUIGamepadBindingAttribute settingsUIGamepadBindingAttribute = default(SettingsUIGamepadBindingAttribute);
		SettingsUIMouseBindingAttribute settingsUIMouseBindingAttribute = default(SettingsUIMouseBindingAttribute);
		if (ReflectionUtils.TryGetAttribute<SettingsUIKeyboardBindingAttribute>((MemberInfo)property, ref settingsUIKeyboardBindingAttribute, false))
		{
			actionName = settingsUIKeyboardBindingAttribute.actionName ?? property.Name;
			device = settingsUIKeyboardBindingAttribute.device;
			type = settingsUIKeyboardBindingAttribute.type;
			component = settingsUIKeyboardBindingAttribute.component;
			control = settingsUIKeyboardBindingAttribute.control;
			modifierControls = settingsUIKeyboardBindingAttribute.modifierControls;
		}
		else if (ReflectionUtils.TryGetAttribute<SettingsUIGamepadBindingAttribute>((MemberInfo)property, ref settingsUIGamepadBindingAttribute, false))
		{
			actionName = settingsUIGamepadBindingAttribute.actionName ?? property.Name;
			device = settingsUIGamepadBindingAttribute.device;
			type = settingsUIGamepadBindingAttribute.type;
			component = settingsUIGamepadBindingAttribute.component;
			control = settingsUIGamepadBindingAttribute.control;
			modifierControls = settingsUIGamepadBindingAttribute.modifierControls;
		}
		else if (ReflectionUtils.TryGetAttribute<SettingsUIMouseBindingAttribute>((MemberInfo)property, ref settingsUIMouseBindingAttribute, false))
		{
			actionName = settingsUIMouseBindingAttribute.actionName ?? property.Name;
			device = settingsUIMouseBindingAttribute.device;
			type = settingsUIMouseBindingAttribute.type;
			component = settingsUIMouseBindingAttribute.component;
			control = settingsUIMouseBindingAttribute.control;
			modifierControls = settingsUIMouseBindingAttribute.modifierControls;
		}
		else
		{
	...
	private bool TryGetSourceBindingForMimic(PropertyInfo property, InputManager.DeviceType device, ActionComponent component, out ProxyBinding sourceBinding)
	{
		...
		SettingsUIBindingMimicAttribute settingsUIBindingMimicAttribute = default(SettingsUIBindingMimicAttribute);
		if (!ReflectionUtils.TryGetAttribute<SettingsUIBindingMimicAttribute>((MemberInfo)property, ref settingsUIBindingMimicAttribute, false))
		{
			return false;
		}
		if (!InputManager.instance.TryFindAction(settingsUIBindingMimicAttribute.map, settingsUIBindingMimicAttribute.action, out var action) || !action.isBuiltIn)
		{
			return false;
		}
		...
		foreach (PropertyInfo propertyInfo in array)
		{
			...
			if (item.m_Source == null)
			{
				...
				if (dictionary2.TryGetValue((binding.actionName, binding.device), out var value2))
				{
					...
					compositeInstance.interactions.AddRange(((IEnumerable<string>)value2.interactions).Select((Func<string, NameAndParameters>)NameAndParameters.Parse));
					compositeInstance.processors.AddRange(((IEnumerable<string>)value2.processors).Select((Func<string, NameAndParameters>)NameAndParameters.Parse));
				}
				else
				{
