...
public class LoadingScreen : IScreenState
{
	public async Task Execute(GameManager manager, CancellationToken token)
	{
		...
		{
			array[0] = TaskManager.instance.GetTaskProgress((Group)1);
			array[1] = TaskManager.instance.GetTaskProgress((Group)2);
			array[2] = TaskManager.instance.GetTaskProgress((Group)3);
